<template>
  <div class="goal-detail">
    ...
    <div class="detail-bar">
      <button class="detail-bar__back" @click="goBack">
        <ArrowLeft :size="16" />
        <span>Minhas Metas</span>
      </button>
      <div v-if="goalData?.status === 'completed'" class="detail-bar__badge detail-bar__badge--done">
        <CheckCircle2 :size="13" />
        Concluída
      </div>
      <div v-else class="detail-bar__badge">
        <Target :size="13" />
        Em andamento
      </div>
    </div>
    ...
    <div v-if="loading" class="detail-loading">
      <span class="detail-loading__spinner" />
      Carregando meta...
    </div>

    <!-- Not found -->
    <div v-else-if="!goalData" class="detail-empty">
    ...
    <template v-else>
      ...
      <div class="goal-header">
        ...
        <div class="goal-header__info">
          <h1 class="goal-header__title">{{ goalData.title }}</h1>
          <p v-if="goalData.description" class="goal-header__desc">{{ goalData.description }}</p>
          <p class="goal-header__plan">{{ goalData.planTitle }}</p>
        </div>
      </div>
      ...
      <div class="progress-section">
        ...
        <span class="progress-label">
          {{ doneCount }}/{{ totalCount }} tarefa{{ totalCount !== 1 ? 's' : '' }} concluída{{ doneCount !== 1 ? 's' : '' }}
        </span>
      </div>
      ...
      <div class="task-list">
        <div
          ...
          class="task-item"
          :class="{
            'task-item--done': isTaskDone(task.id),
        ...
        >
          ...
          <button
            class="task-check"
            :class="{ 'task-check--done': isTaskDone(task.id) }"
            :disabled="markingTask === task.id"
            @click="handleToggleTask(task.id)"
          >
            <Loader2 v-if="markingTask === task.id" :size="13" class="task-check__spinner" />
            <Check v-else-if="isTaskDone(task.id)" :size="13" />
          </button>
          ...
          <div class="task-body">
            ...
            <p class="task-body__title" :class="{ 'task-body__title--done': isTaskDone(task.id) }">
              {{ task.title }}
            </p>
...
import {
  ArrowLeft, Target, Check, ExternalLink, Scale,
  BookOpen, FileText, HelpCircle, Video, RefreshCw,
  MoreHorizontal, PartyPopper, CheckCircle2, Loader2,
...
const goalData = computed(() =>
  studentStore.myGoals.find(
    g => g.enrollmentId === enrollmentId.value && g.goalProgressId === goalProgressId.value
  )
)

const tasks = computed(() =>
  goalData.value ? studentStore.getGoalTasks(goalData.value) : []
)

const totalCount = computed(() => tasks.value.length)
const doneCount = computed(() =>
  tasks.value.filter(t => isTaskDone(t.id)).length
)
const progressPct = computed(() =>
  totalCount.value ? (doneCount.value / totalCount.value) * 100 : 0
)
...
function isTaskDone(taskId) {
  return goalData.value ? studentStore.isTaskDone(goalData.value, taskId) : false
}
...
async function handleToggleTask(taskId) {
  ...
  markingTask.value = taskId
  try {
    await studentStore.toggleTaskDone(
      goalData.value.enrollmentId,
      goalData.value.goalProgressId,
      taskId,
    )
  } finally {
    markingTask.value = null
  }
}

function goBack() {
  router.push({ name: 'StudentGoals' })
}
...
function openLeiSeca(task) {
  ...
  router.push({
    ...
    params: {
      ...
      goalProgressId: goalProgressId.value,
      taskId: task.id,
    },
  })
}
...
onMounted(async () => {
  try {
    await studentStore.loadStudentData()
  } finally {
    loading.value = false
  }
})
...
.detail-bar__back:hover { background: #f5f4f0; color: #1a1a2e; }
...
.task-check--done .task-check__spinner {
  color: #fff;
}
...
@media (max-width: 600px) {
  .goal-detail { padding: 16px 12px 40px; }
